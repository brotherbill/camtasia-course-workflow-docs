# Use Case: Recording on Mac, Initializing on Windows

This scenario covers recording your lesson on a Mac using Camtasia Recorder for Mac, then transferring and initializing the project for editing on Windows using Camtasia. The workflow is explicit and rationale-rich.
...
1. Dropbox, OneDrive, Google Drive, or a USB drive is available on both machines (Mac for recording, Windows for editing), either installed or accessed via a web browser.

2. Camtasia (latest version) is installed and activated on both machines (Camtasia for Mac on the Mac, Camtasia for Windows on the Windows PC). (A free trial is available; this course works with the trial version.)
...
## Machine A (Recording on Mac)

### 1. Prepare Your Folder Structure

1. In Finder on Machine A (Mac), create the following empty folder for your lesson: 
   - `~/Movies/camtasia_transfer/lesson_01/`
2. Open: `~/Movies/camtasia_transfer/lesson_01/`
3. Add folders: RAW,  EDIT, EXPORT, FINAL and DOCS

### 2. Launch Camtasia Editor Recorder (on Mac)
...
### 3. Do a "Show and Tell" Recording (on Mac)

1. Click the big red “Rec” button to begin recording.
2. Narrate and demonstrate your lesson content.
3. When finished, click “Rec” button (once or twice to make it stop)
4. This will bring up Camtasia Rev app.
...
3. File > Save
   ...
   1. File path:  `~/Movies/camtasia_transfer/lesson_01/RAW`

   2. File name: lesson_01

   3. Click: Save

4. Do not start editing yet. Instead, immediately dehydrate (archive) your raw recording and project files:

   1. In Camtasia (on Mac), go to File > Export > Project for Windows...
      - This step is required for cross-platform compatibility. It creates a Windows-compatible Camtasia zipped project file that can be opened on a Windows PC.
      - In Camtasia for Windows, the equivalent is File > Export > Zipped Project...
      - Save Zip As: `lesson_01_CAMTASIA_PROJECT.zip`
      - File path: `~/Movies/camtasia_transfer/lesson_01/EDIT/`
      - Click Save button
...
5. Click play to have sanity check that there is video and audio.

6. Close Camtasia Editor
...
1. In Finder, navigate to:  `Movies\camtasia_transfer\lesson_01\`
   1. Select all files with <kbd>Command</kbd> + <kbd>A</kbd>

2. Right-click on any of the selected folders, then select: "Compress".

3. Rename that file to: `lesson_01_ALL.zip`

4. Upload that file from Machine A (Mac) to Dropbox (or your chosen transfer method). This is the only file you need to archive and transfer.
   1. I saved it in DropBox root folder `Camtasia and Audiate course`

5. Wait for Dropbox (or your chosen transfer method) to fully sync the zip file to the cloud or USB drive.

6. You may now delete `~/Movies/camtasia_transfer/lesson_01/` from Machine A (Mac), as your files are backed up to Dropbox.

7. You may close Camtasia Editor
...
## Machine B (Windows - editing)

### 1. Download Camtasia Project (from Machine A - Mac)

1. On Machine B (Windows), ensure Dropbox (or your transfer method) has finished syncing and the zip file is available locally.
...
### 2. Rehydrate Camtasia Project (from Machine A - Mac)

1. Open Camtasia for Windows.

2. Click: New Project

3. File > Import > Zipped Project...
   1. Zipped project file to import: 
      1. File path:  `Videos\lesson_01\EDIT\`
      2. File name: `lesson_01_CAMTASIA_PROJECT.zip`
      3. Click: Open

   2. Import into project directory: `Videos\lesson_01\EDIT`

   3. Click: Import button

4. Click play to sanity test that you have the video and audio working.
